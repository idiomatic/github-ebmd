...
app.use (req, res, next) ->
    {redirect_uri} = req
    {token, state} = req.session ? {}
    {access_token} = token ? {}
    req.github =
        authorize_uri: urlTweak 'https://github.com/login/oauth/authorize',
        ...
        getAll: (uri, query, cb) ->
            ...
            for page in [1..max_pages]
                query.page = page
                await req.github.get uri, query, defer err, items
                return cb err if err
        ...
        issues: (args..., cb) ->
            ...
            req.github.getAll uri, query, cb
        comments: (args..., cb) ->
...
app.get '/', (req, res, next) ->
    {token} = req.session
    {authorize_uri} = req.github
    if token
        err = {}
        await
            req.github.repos defer err.repos, repos
            req.github.orgs defer err.orgs, orgs
        await
            for org in orgs
                {login} = org
                req.github.repos {org:login}, defer err[login], org.repos
        await
            for org in orgs
                {login} = org
                for repo in org.repos
                    ...
                    req.github.milestones {repo:full_name}, defer err[full_name], repo.milestones
        await
            for org in orgs
                for repo in org.repos
                    {full_name} = repo
